...
#### Data Preparation ####
#Read in data:
sapdata <- read.csv('/Users/annalenafink/Deeplearning/yahoo_sap.csv', header=T, stringsAsFactors=F)
sapdata$Date = as.Date(sapdata$Date) #set date in according format

#add timesteps t(t1=1, t2=2,...,tn=n) for regression model:
sapdata <- cbind(sapdata, "observation"=1:nrow(sapdata)) 

#set limit for training and test data (ratio: 80/20):
training_size <- ceiling(0.8*nrow(sapdata)) 
...
training_data <- sapdata[1:training_size,] 
...
testing_data <- sapdata[(training_size+1):nrow(sapdata),]
...
ggplot ()+
  labs(title="Linear Regression: SAP SE", x="time in t", y="Closing Price in EUR", col="") + 
...
#Stationary test:
adf.test(diff(log(sapdata[,4])), alternative="stationary", k=0)
...
predicted <- forecast(arima_model,h=nrow(testing_data)) 
predicted_tmp=as.numeric(predicted$mean)
testing_data$Prediction=predicted_tmp

#Plot testing and training and prediction:
ggplot ()+
  labs(title=(expression(atop("ARIMA (1,1,0): SAP SE"))), x="time in t", y="Closing Price in EUR", col="") +
