...
sap_data = yf.download('SAP.DE', start='2017-01-01', end='2021-12-30')
sap_data.head()
...
plt.title('Stock Prices History: SAP SE') ##Set the plot title
plt.plot(sap_data['Close'], color=(0.1,0.7,0.2)) #Use the Matplotlib plot method to create a line chart for historical close prices of SAP Stock
plt.xlabel('Date') #Set the x-axis labels
plt.ylabel('Prices in EUR') #Set the y-axis labels
plt.savefig('sap_price_history.pdf')   #Save as pdf
...
plt.figure(figsize=(15, 6)) #Set the plot figure size
plt.title('Sales Volume History: SAP SE') ##Set the plot title
plt.plot(sap_data['Volume'], color=(0.1,0.4,0.3)) #Use the Matplotlib plot method to create a line chart for historical close prices of SAP Stock
plt.xlabel('Date') #Set the x-axis labels
plt.ylabel('Volume') #Set the y-axis labels
plt.savefig('sap_sales_history.pdf')   #Save as pdf
...
#Create an empty list for a sequence of feature data (x_train)
x_training = []

#Create an empty list for a sequence of label data (y_train).
y_training = []

#Create a 304-days window of historical prices as our feature data (x_training) and the following 304-days window as label data (y_training):
...
data = sap_data.filter(['Close'])
train = data[:training_data_len]
validation = data[training_data_len:]
validation['Predictions'] = predictions
plt.figure(figsize=(16,8))
plt.title('LSTM: SAP SE')
plt.xlabel('Date')
plt.ylabel('Close Price in EUR')
plt.plot(train)
plt.plot(validation[['Close', 'Predictions']])
plt.legend(['Train', 'Val', 'Predictions'], loc='lower right')
plt.savefig('sap_lstm1.pdf',format="pdf",)   #Save as pdf
plt.show()
